...
module.exports = (options) => {
  ...
  return (files, metalsmith, done) => {
    ...
    async.mapLimit(filenamesAndLinks, options.parallelism, (filenameAndLink, callback) => {
      ...
      const filename = filenameAndLink.filename;
      const link = filenameAndLink.link;

      // Validate links with a protocol (remote links)
      const linkUrl = urlParse(link);
      if (linkUrl.protocol) {
        if (protocolValidators[linkUrl.protocol] !== undefined) {
          const valid = protocolValidators[linkUrl.protocol](link, options, callbackResult);
          if (valid === undefined) {
          ...
          callbackResult(null, valid ? null : 'not found');
          return;
        }
